...
function wireCartUI() {
  ...
  function refreshOrderText() {
    ...
    // --- NEW COMPACT FORMAT START ---
    const itemStrings = cart.items.map(it => {
    ...
    // Join with double space for readability, but keep on same lines to save paper
    if (itemStrings.length > 0) {
      lines.push(itemStrings.join("  "));
    }
    // --- NEW COMPACT FORMAT END ---
